...
def cornersHeuristic(state, problem):
    ...
    while nothasbeen:
        distance = [] # we need cover the old list
        cornerlist = []
        for corner in nothasbeen:
            if node in nothasbeen:
                nothasbeen.remove(node)
            a = math.pow(node[0]-corner[0],2) + pow(node[1]-corner[1],2)
            b = math.pow(a, 0.5)
            distance.append(b)
            cornerlist.append(corner)
        index = distance.index(min(distance))
        value=value + min(distance)
        corner = cornerlist[index]
        node = corner
...
def foodHeuristic(state, problem):
    ...
    total_dist = 0
    for i in unvisited_foods:
        dist = find(current_pos, unvisited_foods, problem)
        total_dist = dist
    ...
    while foods:
        ...
        for food in foods:
            if node in foods:
                foods.remove(node)
                continue
            a = pow(pow(node[0]-food[0],2)+pow(node[1]-food[1]),0.5)
            if len(distance) == 0:
                distance = (a, food)
            elif distance[0] >= a:
                distance = (a, food)
...
def find(current_pos, corners, problem):
    max_dist=0
    for i in range(len(corners)):
      #a = math.pow(current_pos[0] - corners[i][0], 2) + pow(current_pos[1] - corners[i][1], 2)
      #b = math.pow(a, 0.5)
      dist = mazeDistance(current_pos, corners[i], problem.startingGameState)
...
class ApproximateSearchAgent(Agent):
    ...
    def getAction(self, state):
        ...
        return search.aStarSearch(problem, foodHeuristic(state, problem))
