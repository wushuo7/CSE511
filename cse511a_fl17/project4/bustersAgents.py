...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    #first find the cloest ghost
    Ghost = None
    min = int(100000)
    for i in livingGhostPositionDistributions:
      mostPossibleLocation = i.argMax()
      distance = self.distancer.getDistance(mostPossibleLocation, pacmanPosition)
      if distance <min:
        min = distance
        cloest = mostPossibleLocation
    move = None
    min2 =int(1000)
    for action in legal:
      successorPosition = Actions.getSuccessor(pacmanPosition, action)
      distance2 = self.distancer.getDistance(successorPosition,cloest)
    #second find the best action leads the Pacman to the ghost
      if distance2  <min2:
        min2 = distance2
        best = action
    return best
