...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #print  "1", noisyDistance
    #print  "2", emissionModel
    #print  "3", pacmanPosition
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    all = util.Counter()
    for now in self.legalPositions:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, now))
      #newPostDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
      for newPos, prob in newPosDist.items():
        all[newPos] = all[newPos]+ prob* self.beliefs[now]
    all.normalize()
    self.beliefs = all  
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    while total >0:
      for i in self.legalPositions:
        if total >0:
          #put the particles uniformly
          self.listofparticle.append(i)
          total = total -1
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance != None:
      ...
      for i in self.listofparticle:
        trueDistance = util.manhattanDistance(i, pacmanPosition)
        allPossible[i] = emissionModel[trueDistance]*OldB[i]
      if allPossible == None:
        #recreated 
        self.initializeUniformly(gameState)
      else:
        ...
        for k in range(self.numParticles):
          newB.append(util.sample(allPossible))
        self.listofparticle = newB
    else:#capture a ghost
      self.listofparticle=[self.getJailPosition()]*self.numParticles
    "*** YOUR CODE HERE ***"
    #self.beliefs = allPossible
  def elapseTime(self, gameState):
    """
    ...
    "*** YOUR CODE HERE ***"
    all = []
    for oldPos in self.listofparticle:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      #newPostDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
      all.append(util.sample(newPosDist))
    
    self.listofparticle = all
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(a):
      ...
      b = self.numGhosts
      for j in range(b):
        location.append(random.choice(self.legalPositions))
      right_now.append(tuple(location))
    self.particles= right_now
